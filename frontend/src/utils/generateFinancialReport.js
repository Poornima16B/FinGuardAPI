...
export async function generateFinancialReport({
  ...
  doc.text(generatedAt, pageWidth - margin, 22, { align: 'right' });
  ...
  const summaryLines = [
    ...
    `${t('pdf.fsiScore')}: ${dashboardData.stabilityScore?.toFixed?.(1) ?? dashboardData.stabilityScore} (${dashboardData.riskLevel} ${t('pdf.riskSuffix')})`,
  ];
