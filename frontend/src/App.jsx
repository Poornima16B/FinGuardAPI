...
const FALLBACK_DASHBOARD = {
  totalIncome: 28450.0,
  totalExpense: 19280.5,
  balance: 9169.5,
  stabilityScore: 78.2,
  riskLevel: 'LOW',
  savingsPotential: 3200.0,
  recentTransactions: [
    { id: 1, type: 'INCOME', amount: 3200.0, category: 'FREELANCE', date: '2026-05-18', description: 'Upwork - UI Redesign' },
    { id: 2, type: 'INCOME', amount: 2200.0, category: 'FREELANCE', date: '2026-05-17', description: 'Fiverr - Mid-month milestone' },
    { id: 3, type: 'EXPENSE', amount: 1500.0, category: 'RENT', date: '2026-05-17', description: 'Monthly rent', accountName: 'Primary Bank', accountType: 'BANK', upiId: '' },
    { id: 4, type: 'INCOME', amount: 1850.0, category: 'GIG_WORK', date: '2026-05-15', description: 'Fiverr - Branding Package' },
    { id: 5, type: 'EXPENSE', amount: 1650.0, category: 'SHOPPING', date: '2026-05-12', description: 'Apple MacBook Air', accountName: 'UPI Wallet', accountType: 'UPI', upiId: 'primary@upi' },
    { id: 6, type: 'INCOME', amount: 1200.0, category: 'BUSINESS', date: '2026-05-10', description: 'Web Retainer' },
    { id: 7, type: 'EXPENSE', amount: 185.50, category: 'BILLS', date: '2026-05-09', description: 'AWS Hosting & Tools', accountName: 'Secondary Bank', accountType: 'BANK', upiId: '' },
    { id: 8, type: 'INCOME', amount: 1500.0, category: 'FREELANCE', date: '2026-05-01', description: 'Retainer Client A' },
    { id: 9, type: 'EXPENSE', amount: 4200.0, category: 'SHOPPING', date: '2026-04-28', description: 'Home office setup', accountName: 'Primary Bank', accountType: 'BANK', upiId: '' },
    { id: 10, type: 'EXPENSE', amount: 142.0, category: 'FOOD', date: '2026-05-18', description: 'Daily food expenses', accountName: 'UPI Wallet', accountType: 'UPI', upiId: 'primary@upi' },
  ],
  ...
  const handleSaveProfile = async () => {
    ...
    if (!backendOffline && token) {
      try {
        ...
        const mergedUser = { ...user, fullName: updated.fullName, profession: updated.profession, targetSavings: updated.targetSavings };
        setUser(mergedUser);
      ...
      <aside className={`finguard-sidebar ${isSidebarOpen ? 'sidebar-open' : ''}`}>
        ...
        <div className="sidebar-brand-center">
          ...
          <h2 className="sidebar-app-name">
            ...
            <span className="sidebar-name-ai">AI</span>
          </h2>
        ...
        <nav className="sidebar-nav">
          ...
          <button
            type="button"
            className="sidebar-nav-item nav-insights btn-secondary"
            onClick={() => {
              setCurrentTab('dashboard');
              setTimeout(() => {
                document.getElementById('ai-spending-insights')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
              }, 120);
            }}
          >
      ...
      <main style={{ flexGrow: 1, padding: '3.5rem 4rem', overflowY: 'auto', height: '100vh', position: 'relative', zIndex: 1 }}>
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              ...
              <div className="glass-panel profile-section-card profile-accent-emerald">
                  ...
                  {profileDraft.savingsGoals.map((goal, index) => (
                    <div key={`${goal.name}-${index}`} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      ...
                      <label style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600 }}>{t('profile.targetInr')}</span>
                        <input 
                          type="number" 
                          className="form-input" 
                          disabled={!isProfileEditing} 
                      ...
                      <label style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600 }}>{t('profile.monthlyPlanInr')}</span>
                        <input 
                          type="number" 
                          className="form-input" 
                          disabled={!isProfileEditing} 
                      ...
                      <div style={{ marginTop: '0.25rem', paddingTop: '0.75rem', borderTop: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                          <span>{t('profile.monthlyContribution')}</span>
                          <strong>{formatCurrency(goal.monthlyPlan, 0)}</strong>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                          <span>{t('profile.targetBuffer')}</span>
                      ...
                      {isProfileEditing && (
                        ...
                        >
                          <Trash2 size={14} style={{ color: 'var(--color-danger)' }} /> {t('profile.removeGoal')}
                        </button>
                      )}
                    </div>
                  ))}
                ...
                {isProfileEditing && (
                  <button className="btn-secondary" type="button" onClick={() => addProfileListItem('savingsGoals', { name: t('profile.defaultNewGoal'), category: 'HOUSE', targetAmount: 10000, monthlyPlan: 1000 })} style={{ marginTop: '1.25rem', alignSelf: 'flex-start', padding: '0.75rem 1.5rem' }}>
                    <Plus size={16} /> {t('profile.addSavingsGoal')}
                  </button>
                )}
              </div>
        ...
        {currentTab === 'dashboard' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
            ...
            {/* Cards aggregate */}
            <div ref={dashboardReportRef} className="dashboard-metric-row" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '1.25rem' }}>
            ...
            <div id="ai-spending-insights">
              <AiSpendingInsights dashboardData={dashboardData} user={user} t={t} />
            </div>
        ...
        {currentTab === 'income' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
              ...
              {/* Box 1: Hourly - Indigo */}
              <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', background: 'linear-gradient(135deg, #4f46e5 0%, #6366f1 100%)', border: '1px solid rgba(255,255,255,0.15)' }}>
                <div>
                  <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.75)' }}>{t('income.hourlyRate')}</span>
                  <h3 style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem', color: '#fff' }}>
                    {formatCurrency(dashboardData.totalIncome / 160)}
                  </h3>
                  <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.6)' }}>{t('income.basedOn160Hours')}</span>
                </div>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(255,255,255,0.2)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Clock size={20} />
                </div>
              </div>

              {/* Box 2: Daily - Emerald */}
              <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', background: 'linear-gradient(135deg, #059669 0%, #10b981 100%)', border: '1px solid rgba(255,255,255,0.15)' }}>
                <div>
                  <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.75)' }}>{t('income.dailyAverageRate')}</span>
                  <h3 style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem', color: '#fff' }}>
                    {formatCurrency(dashboardData.totalIncome / 30)}
                  </h3>
                  <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.6)' }}>{t('income.basedOn30Days')}</span>
                </div>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(255,255,255,0.2)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Calendar size={20} />
                </div>
              ...
              {/* Box 3: Weekly - Rose */}
              <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', background: 'linear-gradient(135deg, #e11d48 0%, #f43f5e 100%)', border: '1px solid rgba(255,255,255,0.15)' }}>
                <div>
                  <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.75)' }}>{t('income.weeklyAverageRate')}</span>
                  <h3 style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem', color: '#fff' }}>
                    {formatCurrency(dashboardData.totalIncome / 4.33)}
                  </h3>
                  <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.6)' }}>{t('income.basedOn433Weeks')}</span>
                </div>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(255,255,255,0.2)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <TrendingUp size={20} />
                </div>
              ...
              {/* Box 4: 6-Month - Amber */}
              <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', background: 'linear-gradient(135deg, #d97706 0%, #f59e0b 100%)', border: '1px solid rgba(255,255,255,0.15)' }}>
                <div>
                  <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.75)' }}>{t('income.sixMonthProjection')}</span>
                  <h3 style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem', color: '#fff' }}>
                    {formatCurrency(dashboardData.totalIncome * 6)}
                  </h3>
                  <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.6)' }}>{t('income.aggregateVelocity')}</span>
                </div>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(255,255,255,0.2)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Landmark size={20} />
                </div>
